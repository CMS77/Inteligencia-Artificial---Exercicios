...
# Define a depth-first search function with state checking and path tracking
def dfs_cverificacao(graph, node, visited, target, path):
    ...
    if node not in visited:
        ...
        for neighbor in graph.neighbors(node):
            result = dfs_cverificacao(graph, neighbor, visited, target, path)
...
# Track the chosen path
chosen_path = dfs_cverificacao(G, node=0, visited=visited_with_state_check, target=no_final, path=[])
